module.exports = {
    ...
    adminBookings: `SELECT bs.id_book_services bookingId,bs.id_user,bs.user_mobile_number,bs.service_name,bs.time_slot,DATE_FORMAT(bs.day_slot, '%d-%b,%Y') day_slot,bs.status,up.full_name, ua.full_address, ua.city FROM booked_services bs LEFT JOIN user_profile up ON up.id_user = bs.id_user LEFT JOIN user_address ua ON ua.id_user = bs.id_user WHERE bs.status= 'Active' GROUP BY bs.id_book_services ORDER BY bs.day_slot DESC;`,
